...
//      uint64_t tile_dim_size = 512;
      for (uint64_t r_i = 0; r_i < num_repeats; ++r_i) {
        ...
        std::cout << "FAM tiling dim_size: " << dim_size << ", tile_dim_size: " << tile_dim_size << ", r_i: " << r_i << ", time_exe(s): " << tt_duration.count()
                  << std::endl;
...
//    std::string tiling_filename = std::format("output.gemm.dram.tiling.size{}.log", dim_size);
      ...
      {
        std::stringstream ss;
        ss << "output.gemm.fam.tiling.matrix-size-" << dim_size << ".tile-size-" << tile_dim_size << ".log";
  ...
  std::string collect_filename("output.gemm.fam.collection.tile-sizes.seq.csv");
  std::ofstream fout;
...
//    std::string header("Matrix_size,RAPID.No-Tiling(s),RAPID.Tiling(s)");
    ...
    for (uint64_t tile_dim_size: tile_dim_sizes) {
      header += ",RAPID.Tile-size-" + std::to_string(tile_dim_size);
    }
