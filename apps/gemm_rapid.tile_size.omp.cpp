...
//    std::string tiling_filename = std::format("output.gemm.dram.tiling.size{}.log", dim_size);
        ...
        {
          std::stringstream ss;
          ss << "output.gemm.fam.tiling.matrix-size-" << dim_size << ".tile-size-" << tile_dim_size << ".thread-" << num_thd << ".log";
...
//      gemm_tiling_avg_times.push_back(tiling_avg_time);
    ...
    { /// Tiling file
      std::string collect_filename = "output.gemm.fam.collection.matrix-size-" + std::to_string(dim_size) + ".tile-sizes.num-threads.csv";
      std::ofstream fout;
...
//      std::string header("Matrix_size,DRAM.No-Tiling(s),DRAM.Tiling(s)");
        ...
        for (uint64_t num_thd: num_threads) {
          header += ",RAPID.Thread-" + std::to_string(num_thd);
        }
