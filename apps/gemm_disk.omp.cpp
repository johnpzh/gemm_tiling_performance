...
//  for (uint64_t dim = 64; dim <= 2048; dim *= 2) {
  for (uint64_t dim = 1024; dim <= 16384; dim *= 2) {
...
//  }
  ...
  for (uint64_t num_thd: num_threads) {
    ...
    for (uint64_t dim_size: dim_sizes) {
      ...
      uint64_t C1 = A1;
      uint64_t C2 = B2;
      double *A = create_matrix_in_disk(A1, A2, 1.1);
      double *B = create_matrix_in_disk(B1, B2, 2.2);
      double *C = create_matrix_in_disk(C1, C2);
...
//                    C);
      ...
      destroy_matrix_in_disk(A, A1, A2);
      destroy_matrix_in_disk(B, B1, B2);
      destroy_matrix_in_disk(C, C1, C2);
      /// Save all exe times
//    std::string no_tiling_filename = std::format("output.gemm.dram.no-tiling.size{}.log", dim_size);
      std::string no_tiling_filename;
      {
        std::stringstream ss;
        ss << "output.gemm.disk.no-tiling.matrix-dim-size-" << dim_size << ".thread-" << num_thd << ".log";
  ...
  { /// No-tiling file
    std::string collect_filename("output.gemm.disk.no-tiling.collection.csv");
    std::ofstream fout;
    fout.open(collect_filename);
    if (fout.is_open()) {
      std::string header("Matrix_size");
      for (uint64_t num_thd: num_threads) {
        header += ",DISK.No-Tiling.Thread-" + std::to_string(num_thd);
      }
