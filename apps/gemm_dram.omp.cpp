...
//              C);
        ...
        std::cout << "DRAM no-tiling dim_size: " << dim_size << ", num_threads: " << num_thd << ", r_i: " << r_i << ", time_exe(s): "
...
//    std::string no_tiling_filename = std::format("output.gemm.dram.no-tiling.size{}.log", dim_size);
      ...
      {
        std::stringstream ss;
        ss << "output.gemm.dram.no-tiling.matrix-dim-size-" << dim_size << ".thread-" << num_thd << ".log";
...
//                     C);
        ...
        auto tt_end = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> tt_duration = tt_end - tt_start;
        std::cout << "DRAM tiling dim_size: " << dim_size << ", num_threads: " << num_thd << ", r_i: " << r_i << ", time_exe(s): "
                  << tt_duration.count() << std::endl;
...
//    std::string tiling_filename = std::format("output.gemm.dram.tiling.size{}.log", dim_size);
      ...
      {
        std::stringstream ss;
        ss << "output.gemm.dram.tiling.matrix-dim-size-" << dim_size << ".thread-" << num_thd << ".log";
...
//      gemm_tiling_avg_times.push_back(tiling_avg_time);
  ...
  { /// No-tiling file
    std::string collect_filename("output.gemm.dram.no-tiling.collection.csv");
    std::ofstream fout;
...
//    std::string header("Matrix_size,DRAM.No-Tiling(s),DRAM.Tiling(s)");
      ...
      for (uint64_t num_thd: num_threads) {
        header += ",DRAM.No-Tiling.Thread-" + std::to_string(num_thd);
      }
...
//      fout << dim_sizes[row_i] << "," << gemm_no_tiling_avg_times[row_i] << "," << gemm_tiling_avg_times[row_i] << std::endl;
  ...
  { /// Tiling file
    std::string collect_filename("output.gemm.dram.tiling.collection.csv");
    std::ofstream fout;
    fout.open(collect_filename);
    if (fout.is_open()) {
//    std::string header("Matrix_size,DRAM.No-Tiling(s),DRAM.Tiling(s)");
      std::string header("Matrix_size");
      for (uint64_t num_thd: num_threads) {
        header += ",DRAM.Tiling.Thread-" + std::to_string(num_thd);
      }
      fout << header << std::endl;
      for (uint64_t row_i = 0; row_i < dim_sizes.size(); ++row_i) {
//      fout << dim_sizes[row_i] << "," << gemm_no_tiling_avg_times[row_i] << "," << gemm_tiling_avg_times[row_i] << std::endl;
